...
Question-paper generation uses JSON-object mode with server-owned normalization and validation. It sends a compact balanced evidence excerpt, rejects incomplete/duplicate/non-Hindi or numerically invalid papers, preserves the submitted blueprint and teacher-written questions, and switches across the primary paper model, paper fallback model, and the independently configured general model instead of retrying a broken response against one model pool. Paper instructions are presentation metadata owned by the server: teacher wording is preserved and missing provider instructions are filled deterministically, so an otherwise complete paper is not discarded. Concise small papers use a size-aware Hindi check, while sectionless API requests receive an exact mark distribution when possible.
